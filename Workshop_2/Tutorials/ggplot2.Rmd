...
AmesHousing <- read.csv("/Users/chantalkoechli/Desktop/Rcourse/data/AmesHousing.csv")
head(AmesHousing)
```
...
3) Do these graphs indicate that the quality of a kitchen could be related to the sale price?
4) In the RStudio console, type `?qplot`. Modify the above code to create a barchart (`geom=bar`) to count the number of sales for each level of kitchen quality. What is the deference between the color and fill command?
...
5) Modify the code for histogram above so that the `aes` is not within the `geom`. However the resulting graph should look identical to the one above.
6) Create a scatterplot using `ggplot` with **Fireplaces** as the x-axis and **SalePrice** as the y-axis.
...
7) Create a histogram of the above ground living area, `Gr.Liv.Area`.
8) Create a scatterplot using `Year.Built` as the explanatory variable and `SalePrice` as the response variable. Include a regression line, a title, and labels for the x and y axes.
9) Modify the scatterplot in Question 8) so that there is still only one regression line, but the points are colored by the overall condition of the home, `Overall.Cond`. 
...
10) In the density plot above, explain what the `color`, `fill`, and `alpha` commands are used for. Hint: try running the code with and without these commands or use the [Data Visualization Cheat Sheet]( https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf). 

11) In the boxplot, what is done by the code `coord_flip()`?

12) Create a new boxplot, similar to the one above, but use `theme_bw()` instead of the given theme command. Explain how the graph changes.

13) Use the tab completion feature in RStudio (type theme and hit the `Tab` key to see various options) to determine what theme is the default for most graphs in ggplot.
