...
and load in the example data sets (note that I'm using relative file paths below)
...
A data set satisfying these rules is said to be **tidy**, a term popularized by Hadley Wickham (R god and creator of the tidyverse).
...
**Remark** 
  ...
  This data set started in a relatively tidy form, so it may be difficult to see the benefit of tidying it. Tidy data is typically required for summarizing and plotting data in R. 
...
```{r}
tidy_polls2 <- separate(data = tidy_polls, col = Sample, into = c("size", "population"), sep = " ")
tidy_polls2
```
...
```{r}
tidy_polls3 <- gather(data = tidy_polls2, key = candidate, value = percentage, 7:10)
head(tidy_polls3)
```
...
```{r}
tidy_polls4 <- separate(tidy_polls3, candidate, into= c("candidate", "party"))
head(tidy_polls4)
```
...
2. Let's revisit the dataframe we previously created :

The following data are weights of food (kg) consumed per day by adult deer at different times of the year.
...
If we create the dataframe as follows, it is not a tidy dataframe. Why?
```{r}
Feb <- c(4.7, 4.9, 5.0, 4.8, 4.7, NA) 
May <- c(4.6, 4.4, 4.3, 4.4, 4.1, 4.2)
Aug <- c(4.8, 4.7, 4.6, 4.4, 4.7, 4.8)
Nov <- c(4.9, 5.2, 5.4, 5.1, 5.6, NA)

Deer <- data.frame(Feb, May, Aug, Nov)

```

Tidy the dataframe, using `gather`

3. The file `under5mortality.csv` contains the child mortality rate per 1,000 children born for each country from 1800 to 2015. (Source: https://www.gapminder.org/data/)
...
4. The file `mlb2016.csv` contains the salary information presented by *USA Today* for all 862 players in Major League Baseball. (Source:http://www.usatoday.com/sports/mlb/salaries/2016/player/all/)
     a. Briefly describe why it is not considered to be tidy data and what changes need to be      made to tidy it.
